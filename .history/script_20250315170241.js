...
function generatePretermVariations(preterit) {
    ...
    function changeRandomLetter(word) {
        ...
        // Convertir en tableau de caractères pour faciliter la modification
        const letters = word.split('');
        // Choisir une position aléatoire (excluant le premier caractère pour préserver le début du mot)
        const pos = Math.floor(Math.random() * (letters.length - 1)) + 1;
        // Remplacer par une autre lettre de l'alphabet
    ...
    function swapAdjacentLetters(word) {
        ...
        const letters = word.split('');
        // Choisir une position aléatoire (excluant la première et dernière position)
        const pos = Math.floor(Math.random() * (letters.length - 2)) + 1;
        // Échanger avec la lettre suivante
    ...
    function addOrRemoveLetter(word) {
        ...
        if (add) {
            ...
            const newChar = chars.charAt(Math.floor(Math.random() * chars.length));
            letters.splice(pos, 0, newChar);
        } else {
            // Supprimer une lettre aléatoire (pas la première)
            const pos = Math.floor(Math.random() * (letters.length - 1)) + 1;
    ...
    while (uniqueVariations.length < 3) {
        ...
        do {
            ...
            if (technique === 0) {
                newVariation = changeRandomLetter(baseWord);
            } else if (technique === 1) {
                newVariation = swapAdjacentLetters(baseWord);
            } else {
...
function generateGameOptions(askForPreterit) {
    ...
    if (difficultyMode === 'easy') {
        ...
        if (currentGameOptions.length < 4) {
            ...
            // qui garantit 3 variations, mais on garde ce code par sécurité
